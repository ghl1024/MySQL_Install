...
- 此脚本为 `mysql5.7` 二进制方式自动安装脚本，请核对下载的安装介质是否正确。

- 安装前需要将脚本和安装介质传输到相同任意路径下，用 `root` 用户运行脚本，根据提示输入内容安装即可。
...
- 快速默认安装，（软件安装在 `/mysql/mysoft` 下，数据目录在 `/mysql/mydata` 下)；
